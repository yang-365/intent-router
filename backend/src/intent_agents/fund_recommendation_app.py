"""Fund recommendation agent FastAPI application."""
...
def create_app() -> FastAPI:
    application = FastAPI(title="Fund Recommendation Agent", version="0.1.0")
    service = FundRecommendationAgentService()

    @application.get("/health")
    async def health() -> dict[str, object]:
        return {"status": "ok", "service": "fund-recommendation-agent"}

    @application.post("/api/agent/run", response_model=AgentExecutionResponse)
    async def run_agent(request: FundRecommendationAgentRequest) -> AgentExecutionResponse:
